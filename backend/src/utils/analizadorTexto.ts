...
class FiltroPalabras {
  ...
  scan(texto: string): { 
    ...
    // 2️⃣ Detectar patrones de spam (URLs, emails, teléfonos)
    ...
    // 3️⃣ Verificar frases prohibidas completas
    ...
    // 4️⃣ Verificar palabras individuales
    ...
    // 5️⃣ Detectar patrones de spam comercial
...
export class AnalizadorTexto {
  ...
  analizarTexto(texto: string): AnalisisTexto {
    ...
    console.log(`🔍 Ofensivo: ${resultadoScan.esOfensivo}, Spam: ${resultadoScan.esSpam}, Sin sentido: ${resultadoScan.esSinsentido}, Patrones spam: ${resultadoScan.tienePatronesSpam}`);

    // ✅ LÓGICA DE PUNTUACIÓN MEJORADA (MÁS ESTRICTA SIN PERSPECTIVE)
    ...
    // Penalización adicional por patrones de spam
    if (resultadoScan.tienePatronesSpam) {
      puntuacionBase = Math.min(puntuacionBase, 0.3);
      if (esAprobado) {
        esAprobado = false;
    ...
    // Penalización adicional por baja calidad
    if (resultadoScan.calidadTexto.porcentajePalabrasValidas < 0.5) {
      puntuacionBase = Math.min(puntuacionBase, 0.4);
      if (esAprobado && resultadoScan.calidadTexto.porcentajePalabrasValidas < 0.3) {
        esAprobado = false;
    ...
    return {
      ...
      razon,
      detalles: {
        metodo: 'filtro-palabras-mejorado',
        intencion,
        calidadTexto: resultadoScan.calidadTexto,
